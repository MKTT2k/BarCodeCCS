...
namespace Barcode_CCSTape.GUI
{
    public partial class fHOME : Form
    {
        ...
        bool _runMode;
        public static bool _rsScan1 = false;
        bool _rsScan2 = false;
        string _path;
        #endregion
        ...
        private void fHOME_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(lblSerialFPCB_Using.Text))
            {
                Properties.Settings.Default.SerialFPCB_Using = lblSerialFPCB_Using.Text;
            }
        ...
        private void btnConnectPLC_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    _plc.ActLogicalStationNumber = 2;
                    _plc.Open();
                    if (_plc.Open() != -268435453)
                    {
        ...
        private void btnExportText_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                saveFileDialog.FileName = DateTime.Now.ToString("ddMMyyyy") + "_" + lblSerialFPCB_Using.Text;
        ...
        private void btnSaveFPCBInfor_Click(object sender, EventArgs e)
        {
            base.Invoke(new MethodInvoker(delegate ()
            {
                try
                {
                    if (lblSerialFPCB_Using.Text != lblFPCB_Serial.Text)
                    {
                        if (_tbProdOfLot.Rows.Count > 0)
                        {
                            ...
                            {
                                checkProdOfLot(lblFPCB_Serial.Text);
                                lblSerialFPCB_Using.Text = lblFPCB_Serial.Text;
                            }
                        }
                        else
                        {
        ...
        private void dgvProdOf_LOT_Sorted(object sender, EventArgs e)
        {
            for (int i = 0; i < dgvProdOf_LOT.Rows.Count; i++)
            {
                dgvProdOf_LOT.Rows[i].Cells[0].Value = i + 1;
            }
            dgvProdOf_LOT.Rows[0].Cells[0].Selected = false;
        }

        private void dgvLOT_PASS_Sorted(object sender, EventArgs e)
        {
            for (int i = 0; i < dgvLOT_PASS.Rows.Count; i++)
            {
                dgvLOT_PASS.Rows[i].Cells[0].Value = i + 1;
            }
            dgvLOT_PASS.Rows[0].Cells[0].Selected = false;
        }
        #endregion
        #endregion
        ...
        #region Data Process
        /// <summary>
        /// dislay data on dataGridView with automatically numbered rows
        /// </summary>
        ...
        private void checkLOTDB(string scan)
        {
            try
            {
                ...
                DataTable scanned = _dtcnn.get_FPCBAfterCCS(scan);
                if (table != null)
                {
                    lblLOT_Status.Text = "OK";
                    lblLOT_Status.ForeColor = Color.Green;
                    lblFPCB_Serial.Text = table.Rows[0][1].ToString().ToUpper();
                    lblFPCB_Serial.ForeColor = Color.Green;
                    lblLOT_Type.Text = table.Rows[0][5].ToString().ToUpper();
                    lblLOT_Type.ForeColor = Color.Green;
                    lblLOT_Quantity.Text = table.Rows[0][3].ToString().ToUpper();
                    lblLOT_Quantity.ForeColor = Color.Green;
                    if (scanned != null)
                    {
                        lblLOT_QtyScanned.Text = scanned.Rows.Count.ToString();
                        lblLOT_QtyScanned.ForeColor = Color.Green;
                    }
                    else
                    {
                        lblLOT_QtyScanned.Text = "0";
                        lblLOT_QtyScanned.ForeColor = Color.Green;
                    }
                ...
                else
                {
                    ...
                    lblFPCB_Serial.Text = ".........................";
                    lblFPCB_Serial.ForeColor = Color.Red;
                    lblLOT_Type.Text = ".........................";
                    lblLOT_Type.ForeColor = Color.Red;
                    lblLOT_Quantity.Text = "...............";
                    lblLOT_Quantity.ForeColor = Color.Red;
                    lblLOT_QtyScanned.Text = "...............";
                    lblLOT_QtyScanned.ForeColor = Color.Red;
                }
            }
            catch (Exception ex)
            {
        ...
        private void checkProdOfLot(string scan)
        {
            try
            {
                lblSerialFPCB_Using.Text = scan;
                _tbProdOfLot = _dtcnn.get_FPCB(scan);
                _tbProdPass = _dtcnn.get_FPCBAfterCCS(scan);
                if (Int32.TryParse(lblLOT_Quantity.Text, out int i))
                {
                    segTotalQty_Prod.Value = lblLOT_Quantity.Text;
                }
                ...
                else
                {
                    if (dgvProdOf_LOT.Rows.Count > 0)
                    {
                        dgvProdOf_LOT.Rows.RemoveAt(0);
                    }
                    MessageBox.Show("All FPCB have CCS Tape!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    //MessageBox.Show("Could not be found product of Serial Lot!", "ERROR", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            //if (!_rsScan1 && !_rsScan2)
            //{
            //    writeBitToPLC("M253", 1);
            //    return;
            //}
            //else if (!_rsScan1 && _rsScan2)
            //{
            //    writeBitToPLC("M253", 1);
            //    return;
            //}
        ...
        private void CCSProcess(string scan, Label lbl)
        {
            try
            {
                ...
                {
                    DataTable tb = _dtcnn.search_FPCB_Scan(lblSerialFPCB_Using.Text, scan);
                    if (tb != null)
                    {
                        string _equip = lblEquip_Name.Text;
                        string _serial = tb.Rows[0][1].ToString();
                        string _code = tb.Rows[0][2].ToString();
                        string _orderno = tb.Rows[0][3].ToString();
                        ...
                        string _lot = lblSerialFPCB_Using.Text;

                        _dtcnn.add_FPCBCCS(_lot, _equip, _serial, _code, _orderno,
                                    _tape1, _tape2, _tape3, _tape4, _tape5); ;
                        logSave_FPCB(_serial, _code, _orderno, _time);
                        CCSTape();
                        checkProdOfLot(_lot);
                    ...
                    //}
                    //    int c = 0;
                    //    for (int i = 0; i < _tbProdOfLot.Rows.Count; i++)
                    //    {
                    //        if (scan == _tbProdOfLot.Rows[i][1].ToString())
                    //        {
                    //            c++;
                    //            string _equip = lblEquip_Name.Text;
                    //            string _serial = _tbProdOfLot.Rows[i][1].ToString();
                    //            string _code = _tbProdOfLot.Rows[i][2].ToString();
                    //            string _orderno = _tbProdOfLot.Rows[i][3].ToString();
                    //            string _tape1 = lblTape1_Serial.Text;
                    //            string _tape2 = lblTape2_Serial.Text;
                    //            string _tape3 = lblTape3_Serial.Text;
                    //            string _tape4 = lblTape4_Serial.Text;
                    ...
                    //            CCSTape();
                    //            checkProdOfLot(_lot);
                    //            formatText(lbl, "OK", Color.Green, Color.Transparent);
                    //            flag = true;
                    //            break;
                    //        }
                    //    }
        ...
        private void checkFolder(string path)
        {
            try
            {
                ...
                if (Properties.Settings.Default.SerialFPCB_Using != string.Empty)
                {
                    checkLOTDB(Properties.Settings.Default.SerialFPCB_Using);
                    checkProdOfLot(lblFPCB_Serial.Text);
        ...
        private void logSave_FPCB(string serial, string code, string orderno, string time)
        {
            try
            {
                ...
                if (!File.Exists(fname))
                {
                    ...
                    if (_tbProdPass != null)
                    {
                        for (int i = 0; i < _tbProdPass.Columns.Count - 1; i++)
                        {
                            s += _tbProdPass.Columns[i].ColumnName + ",";
                        }
                        s += _tbProdPass.Columns[_tbProdPass.Columns.Count - 1].ColumnName + "\n";
        ...
        private void ExportExcel(string filePath)
        {
            ...
            excelWorkSheet.Cells[1, 1] = "LOT " + lblSerialFPCB_Using.Text + " " + DateTime.Now.ToString("dd/MM/yyyy");
        ...
        private void showDataS1(object sender, EventArgs e)
        {
            ...
            //{
            //    CCSProcess(txtProd01_Scan.Text, lblProd01_Status, _rsScan1);
            //}
        }
        ...
        private void showDataS2(object sender, EventArgs e)
        {
            ...
            //{
            //    CCSProcess(txtProd02_Scan.Text, lblProd02_Status, _rsScan2);
            //}
        }
